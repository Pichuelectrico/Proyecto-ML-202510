...
_console = Console()
LOG_FILE_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), "agents_log.txt")
...
def clear_agents_log():
    try:
        with open(LOG_FILE_PATH, "w", encoding="utf-8") as f:
            f.write("=== Agents Log ===\n\n")
    except Exception as e:
        print(f"Error clearing Agents Log: {e}")
...
def reorder_agents_log():
    ...
    try:
        ...
        print(f"Error reordering Agents Log: {e}")
